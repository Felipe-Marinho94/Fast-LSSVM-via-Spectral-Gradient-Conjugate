...
from numpy import zeros, dot, eye, block, ones, array, concatenate, linalg, sign, random, diag, mean, std, where, exp, sort, delete
...
def filtro_numerics_columns(df):
    '''
    função para considerar apenas colunas numéricas
    INPUT:
        df - Pandas dataframe com as features a serem analisadas;
    
    OUTPUT:
        pandas dataframe somente com as features numéricas
    '''

    df_numerics = df.copy()
    df_numerics = df_numerics.select_dtypes(include = ['float64', 'int64'])
    return df_numerics
...
datasets = ['breast.csv', 'column_2C.csv', 'australian.csv', 'bupa.csv', 'shuttle', 'bank',
            'ionosphere.csv','diabetes.csv', 'SouthGermanCredit.csv', 'haberman.csv', 'adult']

#Definição da função para inserção no pipeline
numerics_columns = FunctionTransformer(filtro_numerics_columns)
...
data_pipeline = Pipeline([('numerics_columns', numerics_columns),
                          ('drop_nan', drop_nan_pipeline),
...
def objective(trial):
        ...
        '''
        classifier_name = trial.suggest_categorical('classifier', ['SVC', 'LSSVM', 'FSLM_LSSVM_improved',
                                                                    'RFSLM_LSSVM'   'LSSVM_ADMM', 'TCSMO_LSSVM',
                                                                        'SCG_LSSVM', 'IP_LSSVM'])
        '''
        classifier_name = k
        ...
        if (classifier_name == 'SCG_LSSVM'):
            param_gamma = trial.suggest_float('param_gamma', 1e-2, 1e2, log=True)
            param_ni = trial.suggest_float('param_ni', 1e-2, 1e2, log=True)
            classifier_obj = SCG_LSSVM(gamma = param_gamma, ni = param_ni)
            
        if (classifier_name == 'CFGSMO_LSSVM'):
            param_gamma = trial.suggest_float('param_gamma', 0.1, 1, log=True)
            param_ni = trial.suggest_float('param_ni', 1, 10, log=True)
            classifier_obj = CFGSMO_LSSVM(gamma = param_gamma, ni = param_ni)
        ...
        if (classifier_name == 'SCG_LSSVM') | (classifier_name == 'CFGSMO_LSSVM'):
            resultados = classifier_obj.fit(X_train_processed, y_train)
            alphas = resultados['mult_lagrange']
...
for dataset in datasets:
        ...
        modelos_dual = {'SCG_LSSVM': SCG_LSSVM(ni = best_params['SCG_LSSVM']['param_ni'],
                                         gamma = best_params['SCG_LSSVM']['param_gamma']),
                        'CFGSMO_LSSVM': CFGSMO_LSSVM(ni = best_params['CFGSMO_LSSVM']['param_ni'],
                                         gamma = best_params['CFGSMO_LSSVM']['param_gamma'])}
